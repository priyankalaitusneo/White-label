...
import com.laitsneo.whitelbl.dto.Client.*;
import com.laitsneo.whitelbl.entity.Admin.Charges;
import com.laitsneo.whitelbl.entity.Admin.LockedFunds;
...
import com.laitsneo.whitelbl.helper.AESEncryptor;
import com.laitsneo.whitelbl.helper.Generator;
import com.laitsneo.whitelbl.helper.IpFetching;
import com.laitsneo.whitelbl.repository.Admin.ChargesRepository;
import com.laitsneo.whitelbl.repository.Admin.LockedFundsRepository;
...
import com.laitsneo.whitelbl.service.EmailService;
...
public class ClientServiceImpl implements ClientService {
	...
	@Override
	public ResponseEntity<?> editProfile(String userId, ClientEditProfileDto dto) {
	    ...
	    try {
	        ...
	        if (dto.getEmail() != null && !dto.getEmail().trim().isEmpty()) {
	            ...
	            newEmail = dto.getEmail().toLowerCase().trim();
	        ...
	        // MOBILE UPDATE
	        // =========================
	        if (dto.getMobileNum() != null && !dto.getMobileNum().trim().isEmpty()) {

	            String newMobile = dto.getMobileNum().trim();

	            if (!newMobile.equals(client.getMobileNum())) {

	                if (clientRepository.existsByMobileNumAndUserIdNot(newMobile, userId)) {
	                    response.put("success", false);
	        ...
	        response.put("message",
	                "Profile updated successfully" +
	                        (emailChanged ? ". Please use new email for next login." : "")
	        );
	        ...
	        logger.error("Error during profile update", e);
	        response.put("success", false);
...
//				map.put("availableBalance", balance.get().getAccountNum());
...
//	}
	...
	private boolean isClientActive(Client c) {
		try {
			return c.getStatus().equalsIgnoreCase("ACTIVE");
		} catch (Exception e) {
...
// Hold Amount - Admin holds specific amount from PayIn transaction
	...
	public ResponseEntity<?> holdPayinAmount(@Valid HoldAmountDto holdAmountDto) {
		...
		try {
			...
			Client client = clientOpt.get();
			BigDecimal currentWalletBalance = client.getWalletBalance();
	...
	@Override
	public String clientOnboard(ClientOnboardDto dto, MultipartFile aadhaarFront, MultipartFile aadhaarBack,
			MultipartFile panCard, MultipartFile gstFile, MultipartFile shopPhoto, MultipartFile profilePhoto) {
		// TODO Auto-generated method stub
		return null;
	}
	...
	public ResponseEntity<?> createClient(
	        ClientOnboardDto dto,
	        MultipartFile aadharFront,
	        MultipartFile aadharBack,
	        MultipartFile panDoc,
	        MultipartFile gstDoc,
	    ...
	    try {
	        ...
	        String clientId = generator.generateClientId();
	        c.setUserId(clientId);

	        c.setName(dto.getMerchantName());
	        c.setEmail(dto.getEmail());
	        c.setMobileNum(dto.getMobileNum());
	        c.setPassword(passwordEncoder.encode(dto.getPassword()));

	        c.setDob(dto.getDob());
	        c.setAadhaarNumber(dto.getAadhaar());
	        c.setPan(dto.getPan());
	        ...
	        c.setHouseNumber(dto.getHouseNumber());
	        c.setLandmark(dto.getLandmark());

	        c.setAadhaarNumber(dto.getAadhaar());
	        c.setPan(dto.getPan());
	        c.setMerchantType(dto.getMerchantType());
	        c.setAccountNum(dto.getAccountNumber());
	        c.setIfscCode(dto.getIfscCode());
	        c.setBankName(dto.getBankName());
	        c.setGst(dto.getGst());
	        c.setCin(dto.getCin());
	        c.setScheme(dto.getScheme());

	        String path = "uploads/";

	        c.setAadhaarFrontPath(saveFile(path, aadharFront));
	        c.setAadhaarBackPath(saveFile(path, aadharBack));
	        c.setPanPath(saveFile(path, panDoc));
	        c.setGstPath(saveFile(path, gstDoc));
	        c.setShopPhotoPath(saveFile(path, shopPhoto));
	        c.setProfilePhotoPath(saveFile(path, profilePhoto));
	...
	private String saveFile(String path, MultipartFile file) throws Exception {
